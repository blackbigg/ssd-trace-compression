...
> 📌 **關鍵發現**：即使在 $z = 2$（$64\times$ 極限壓縮）下，模型的 F1-Score 依然維持在 $0.9772$ 的最高水準，顯示分類任務不需要高維度資料即可精準判定。
...
2. **極致硬體親和度（Edge Deployment Ready）**：僅需 **2 個維度（$64\times$ 壓縮）** 即可實現 $\text{F1-Score} = 0.9772$，極大幅度降低了邊緣控制器（SSD Controller）推論時的 SRAM 記憶體佔用與運算延遲。